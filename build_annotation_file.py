...
parser= OptionParser()
#give complete source path to directory food_data(containing subclass directories of images) in paperspace environment
parser.add_option("-s", "--source", dest="image_dir",help="Path to directory containing images & xml in paperspace", default= os.path.join(os.getcwd(), 'food_data'))#default assumes images are in folder named food_data
parser.add_option("-d","--destination", dest="save_to", help= "Path to save annotation txt file in paperspace", default = os.path.join(os.getcwd(),'annotations.txt'))
...
def main(src_path, dest_path):
    ...
    if all([fil.endswith(('.jpg','.xml')) for fil in os.listdir(src_path)]):#True, When all images are stored in single directory
        ...
        xml_df['filename'] = xml_df[[cols_req[-1], 'filename']].apply(lambda x: os.path.join(src_path,'/'.join(x.astype(str))),axis=1)#'/' for paperspace filesystem
    df_txt = pd.DataFrame()    
